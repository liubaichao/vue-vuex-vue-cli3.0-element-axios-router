      ...
      this.$http.getDetailApi({id:id},{headers:{token:JSON.parse(localStorage.getItem('userInfo')).token}}).then(res => { 
            if(res.data&&res.data.code === 200){    
              ...
              data.invoiceList.map((item)=>{ //发票
                invoice.push({invoiceDate:this.$my.timestampToTime(item.invoiceDate,'YMD'),invoiceQty:item.invoiceQty,invoiceNo:item.invoiceNo,source:item.source,url:item.invoiceUrl,id:item.id})
              })            
      ...
      submitData(formName){        
        ...
        this.$refs[formName].validate((valid) => {
          ...
          if (valid) {
            ...
            this.$http.onlineCheckApi(data,{headers:{token:userInfo.token}}).then(res => { 
                if(res.data&&res.data.code === 200){       
                    this.loading = false;
